...
<template>
  <div :class="[isMobile ? 'mt-2' : 'mt-28']" class=" rounded-md px-4 py-4 ">
    ...
    <div class="w-full md:min-w-[450px]">
      <div class="grid grid-cols-3 gap-x-2 gap-y-3" :style="{	gridTemplateColumns: `repeat(${boxData.length}, minmax(0, 1fr))` }">
        <div class="space-y-4" v-for="item in boxData" :key="item.id">
          <h2 class="text-md text-center">
            ...
            <p class="line-clamp-1 break-all overflow-hidden">{{ item.name }}</p>
          </h2>
          <div class="space-y-e" v-for="box in item.childList" :key="box.id">
						<div  @click="handleClick(box)" class="py-3 flex justify-center items-center rounded cursor-pointer select-none bg-neutral-100  hover:bg-neutral-200 dark:bg-neutral-900 dark:hover:bg-neutral-800" :class="[ isMobile ? 'px-2' : 'px-5']">
      ...
      <div class="mt-4 text-center text-sm text-neutral-400" v-if="boxData?.length">
        点击以上话题，快速与我对话
      </div>
    </div>
  </div>
